...
%rmdpath('/home/jmm_vivobook_asus/DeepGaze_project/eeglab14_1_1b/field_trip/'); %% remove field_trip from eeglab
ind=0;
ccount=1;
ckcount=1;
if sel_emg==1
...
while ind==0
     ...
     if indbout==0
         ...
         for ch=1:32
              close all;
              [erspl{ccount,ch},itcpl{ccount,ch},powbasel{ccount,ch},timescl{ccount,ch},freql{ccount,ch}]=newtimef(EEGL_set{ccount}.data(ch,:),EEGL_set{ccount}.pnts,[EEGL_set{ccount}.xmin EEGL_set{ccount}.xmax]*1000,EEGL_set{ccount}.srate,0,'winsize',150,'nfreqs',1000,'freqs',[0 50],'padratio',32,'plotersp','off','plotitc','off');
              [erspr{ccount,ch},itcpr{ccount,ch},powbaser{ccount,ch},timescr{ccount,ch},freqr{ccount,ch}]=newtimef(EEGR_set{ccount}.data(ch,:),EEGR_set{ccount}.pnts,[EEGR_set{ccount}.xmin EEGR_set{ccount}.xmax]*1000,EEGR_set{ccount}.srate,0,'winsize',150,'nfreqs',1000,'freqs',[0 50],'padratio',32,'plotersp','off','plotitc','off');
         ...
         if sel_emg==1
              ...
              for ch=1:32
                  ...
                  %coh_SO_Ls{ccount,ch}=coh_SO_Ls{ccount,ch}./max(max(coh_SO_Ls{ccount,ch}));
                  %coh_SO_Rs{ccount,ch}=coh_SO_Rs{ccount,ch}./max(max(coh_SO_Rs{ccount,ch}));
                  %coh_ST_Ls{ccount,ch}=coh_ST_Ls{ccount,ch}./max(max(coh_ST_Ls{ccount,ch}));
                  %coh_ST_Rs{ccount,ch}=coh_ST_Rs{ccount,ch}./max(max(coh_ST_Rs{ccount,ch}));
                  %coh_TA_Ls{ccount,ch}=coh_TA_Ls{ccount,ch}./max(max(coh_TA_Ls{ccount,ch}));
                  %coh_TA_Rs{ccount,ch}=coh_TA_Rs{ccount,ch}./max(max(coh_TA_Rs{ccount,ch}));
                  %coh_RF_Ls{ccount,ch}=coh_RF_Ls{ccount,ch}./max(max(coh_RF_Ls{ccount,ch}));
                  %coh_RF_Rs{ccount,ch}=coh_RF_Rs{ccount,ch}./max(max(coh_RF_Rs{ccount,ch}));
             end;
         end;
