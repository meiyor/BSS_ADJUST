...
for ii=1:length(uX)
    f=b==ii;
    [hTemp,statsTemp,data{ii}]=myPlotter(x(f),y(:,f));
    %values_p{ii},values_n{ii}
    H = [H,hTemp];
...
%     if data{2}(kk)>=data{1}(kk) 
%         hp=line([1 2],[data{1}(kk) data{2}(kk)],'Color','blue','LineStyle','-','LineWidth',4);
%         hp.Color(4) = 0.45;
%         countp=countp+1;
%     else
%       hn=line([1 2],[data{1}(kk) data{2}(kk)],'Color','red','LineStyle','-','LineWidth',4);
%       hn.Color(4) = 0.45;
%       countn=countn+1;
%     end;
% end;
...
% countn
%for ll=1:4
%        line([values_p{1}(ll,1) values_p{2}(ll,1)],[values_p{1}(ll,2) values_p{2}(ll,2)],'Color','blue','LineStyle','--','LineWidth',2);
%end;
%for ll=1:3
%        line([values_n{1}(ll,1) values_n{2}(ll,1)],[values_n{1}(ll,2) values_n{2}(ll,2)],'Color','red','LineStyle','--','LineWidth',2);
%end;
hold off
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h,statsOut,thisYY]=myPlotter(X,Y)
    %% val_p,val_n
    ...
    for k=1:length(X)
        ...
        %outlier=isoutlier(thisY,'movmean',50,'ThresholdFactor',0.4);
        thisX=violaPoints(thisX,thisY);
        
%         val_n(1,:)=[thisX(329) thisY(329)];
%         val_n(2,:)=[thisX(395) thisY(395)];
%         val_n(1,:)=[thisX(427) thisY(427)];
%         val_n(2,:)=[thisX(444) thisY(444)];
%         val_n(3,:)=[thisX(248) thisY(248)];
%         
%         val_p(1,:)=[thisX(202) thisY(202)];
%         val_p(2,:)=[thisX(333) thisY(333)];
%         val_p(3,:)=[thisX(302) thisY(302)];
%         val_p(4,:)=[thisX(265) thisY(265)];
%         val_p(4,:)=[thisX(436) thisY(436)];
